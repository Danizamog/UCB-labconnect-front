            ...
            <div className="reservation-focus-grid">
              ...
              <div className="reservation-focus-card">
                ...
                <strong className={`reservation-focus-status ${reservation.status}`}>
                  ...
                  {reservation.status === 'in_progress' ? 'En curso' : null}
                  {reservation.status === 'completed' ? 'Completada' : null}
                  {reservation.status === 'absent' ? 'Ausente' : null}
                  {reservation.status === 'rejected' ? 'Rechazada' : null}
                  {reservation.status === 'cancelled' ? 'Cancelada' : null}
                  {!['pending', 'approved', 'in_progress', 'completed', 'absent', 'rejected', 'cancelled'].includes(reservation.status) ? reservation.status : null}
                </strong>
              </div>
            </div>
