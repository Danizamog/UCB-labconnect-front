...
function UserReserveLabPage({ user, notifications = [], onMarkNotificationAsRead }) {
  ...
  const upcomingReservations = useMemo(
    () => myReservations.filter((item) => {
      ...
      const isInactiveStatus = ['cancelled', 'rejected', 'completed', 'absent'].includes(item.status)
      return !actionState.hasStarted && !isInactiveStatus
    }),
    [myReservations],
  )
  ...
  const validNotifications = useMemo(() => {
    ...
    return notifications.filter((notification) => {
      if (!notification.reservation_id) return true
      return !cancelledReservationIds.has(notification.reservation_id)
    })
  }, [notifications, myReservations])

  const unreadNotificationsCount = useMemo(
    () => validNotifications.filter((notification) => !notification.is_read).length,
    [validNotifications],
  )
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      ...
      const cancelledReservation = await deleteReservation(reservationToCancel.id)
      
      // Actualizar localmente las reservas inmediatamente
